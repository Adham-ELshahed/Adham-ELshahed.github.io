...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/The following comparison functions are available:/g, 
    'The following comparison functions are available:\n\n');
  formatted = formatted.replace(/Comparer functions create a comparer function which can be used to control equality and ordering\./g, 
    'Comparer functions create a comparer function which can be used to control equality and ordering.\n\n');
  ...
  // Step 4: Handle comparer-specific patterns
  const comparerPatterns = [
    { 
      pattern: 'IgnoreCase comparer that uses the current culture',
      needsBullet: false
    },
    { 
      pattern: 'Ordinal comparer that uses ordinal rules',
      needsBullet: false
    },
    { 
      pattern: 'Returns a comparer function given the specified comparison criteria',
      needsBullet: false
    }
  ];
...
// Filter comparer functions
let comparerFunctions = functionsData.filter(func => func.category === 'comparer');
let fixedCount = 0;

console.log(`Found ${comparerFunctions.length} comparer functions to properly format`);
console.log('Applying proper bullet point formatting with correct line breaks and indentation...\n');

comparerFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${comparerFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'comparer') {
    const formattedFunc = comparerFunctions.find(cf => cf.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${comparerFunctions.length} comparer functions`);
