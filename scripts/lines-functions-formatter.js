...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/The following Lines functions are available:/g, 
    'The following Lines functions are available:\n\n');
  formatted = formatted.replace(/Line processing can be controlled using the following options:/g, 
    'Line processing can be controlled using the following options:\n\n');
  ...
  // Step 4: Handle lines-specific patterns
  const linesPatterns = [
    { 
      pattern: 'line ending',
      needsBullet: false
  ...
  // Step 5: Handle encoding patterns common in lines functions
...
// Filter lines functions
let linesFunctions = functionsData.filter(func => func.category === 'lines');
let fixedCount = 0;

console.log(`Found ${linesFunctions.length} lines functions to properly format`);
console.log('Applying proper bullet point formatting with correct line breaks and indentation...\n');

linesFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${linesFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'lines') {
    const formattedFunc = linesFunctions.find(lf => lf.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${linesFunctions.length} lines functions`);
