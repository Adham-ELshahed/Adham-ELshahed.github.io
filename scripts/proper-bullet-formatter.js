...
function formatTextProperly(text) {
  ...
  // Step 4: Handle JSON value representations (for Json.FromValue and similar functions)
  const valueRepresentations = [
    { 
      pattern: 'Null, text and logical values are represented as the corresponding JSON types',
      needsBullet: true
    },
    { 
      pattern: 'Numbers are represented as numbers in JSON, except that #infinity, -#infinity and #null are converted to null',
      needsBullet: true
    },
    { 
      pattern: 'Lists are represented as JSON arrays',
      needsBullet: true
    },
    { 
      pattern: 'Records are represnted as JSON objects',
      needsBullet: true
    },
    { 
      pattern: 'Tables are represented as an array of objects',
      needsBullet: true
    },
    { 
      pattern: 'Dates, times, datetimes, datetimezones and durations are represented as ISO-8601 text',
      needsBullet: true
    },
    { 
      pattern: 'Binary values are represented as base-64 encoded text',
      needsBullet: true
    },
    { 
      pattern: 'Types and functions produce an error',
      needsBullet: true
    }
  ];
  
  valueRepresentations.forEach(item => {
    if (formatted.includes(item.pattern)) {
      // Add 2 spaces before bullet for proper indentation
...
console.log(`Found ${accessDataFunctions.length} access-datafunctions to properly format`);
...
console.log(`\nCompleted proper formatting for all ${accessDataFunctions.length} access-datafunctions`);
