...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/The following Error functions are available:/g, 
    'The following Error functions are available:\n\n');
  formatted = formatted.replace(/Error handling can be controlled using the following functions:/g, 
    'Error handling can be controlled using the following functions:\n\n');
  ...
  // Step 4: Handle error-specific patterns
  const errorPatterns = [
    { 
  ...
  // Step 5: Handle error type patterns common in error functions
...
// Filter error functions
let errorFunctions = functionsData.filter(func => func.category === 'error');
let fixedCount = 0;

console.log(`Found ${errorFunctions.length} error functions to properly format`);
...
errorFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${errorFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'error') {
    const formattedFunc = errorFunctions.find(ef => ef.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${errorFunctions.length} error functions`);
