...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/The following Duration functions are available:/g, 
    'The following Duration functions are available:\n\n');
  formatted = formatted.replace(/Duration values can be represented in the following formats:/g, 
    'Duration values can be represented in the following formats:\n\n');
  ...
  // Step 4: Handle duration-specific patterns and formats
  const durationFormatPatterns = [
    { 
      pattern: 'DD.HH:MM:SS format',
      needsBullet: false
    },
    { 
      pattern: 'ISO 8601 duration format',
      needsBullet: false
    },
    { 
      pattern: 'days.hours:minutes:seconds',
      needsBullet: false
    },
    { 
      pattern: 'timespan format',
      needsBullet: false
    }
  ];
  
  // Step 5: Handle time unit patterns common in duration functions
...
// Filter duration functions
let durationFunctions = functionsData.filter(func => func.category === 'duration');
let fixedCount = 0;

console.log(`Found ${durationFunctions.length} duration functions to properly format`);
console.log('Applying proper bullet point formatting with correct line breaks and indentation...\n');

durationFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${durationFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'duration') {
    const formattedFunc = durationFunctions.find(df => df.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${durationFunctions.length} duration functions`);
