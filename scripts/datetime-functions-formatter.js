...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/DateTime values can be represented in the following formats:/g, 
    'DateTime values can be represented in the following formats:\n\n');
  ...
  // Step 4: Handle datetime-specific patterns and formats
  const datetimeFormatPatterns = [
    { 
      pattern: 'yyyy-MM-dd HH:mm:ss format',
      needsBullet: false
    },
    { 
      pattern: 'ISO 8601 datetime format',
      needsBullet: false
    },
    { 
      pattern: 'UTC datetime',
      needsBullet: false
    },
    { 
      pattern: 'local datetime',
      needsBullet: false
    }
  ];
  
  // Step 5: Handle time zone patterns common in datetime functions
...
// Filter date-and-time functions
let datetimeFunctions = functionsData.filter(func => func.category === 'date-and-time');
let fixedCount = 0;

console.log(`Found ${datetimeFunctions.length} date-and-time functions to properly format`);
console.log('Applying proper bullet point formatting with correct line breaks and indentation...\n');

datetimeFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${datetimeFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'date-and-time') {
    const formattedFunc = datetimeFunctions.find(df => df.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${datetimeFunctions.length} date-and-time functions`);
