...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/The following logical values are supported:/g, 
    'The following logical values are supported:\n\n');
  ...
  formatted = formatted.replace(/Text conversion follows these rules:/g, 
    'Text conversion follows these rules:\n\n');
  ...
  // Step 4: Handle logical-specific patterns
  ...
  // Step 5: Handle logical value patterns common in logical functions
  const logicalValuePattern = /(true|false)\s*:\s*([^.]+\.)/gi;
  let logicalValueMatches = [];
  let logicalMatch;
  
  while ((logicalMatch = logicalValuePattern.exec(formatted)) !== null) {
    logicalValueMatches.push({
      fullMatch: logicalMatch[0],
      name: logicalMatch[1],
      description: logicalMatch[2].trim()
    });
  }
  
  if (logicalValueMatches.length > 0) {
    logicalValueMatches.sort((a, b) => b.fullMatch.length - a.fullMatch.length);
    
    logicalValueMatches.forEach(option => {
      formatted = formatted.replace(option.fullMatch, `\n  • ${option.name}: ${option.description}`);
    });
  }
...
let logicalFunctions = functionsData.filter(func => func.category === 'logical');
let fixedCount = 0;

console.log(`Found ${logicalFunctions.length} logical functions to properly format`);
console.log('Applying proper bullet point formatting with correct line breaks and indentation...\n');

logicalFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${logicalFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'logical') {
    const formattedFunc = logicalFunctions.find(lf => lf.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${logicalFunctions.length} logical functions`);
