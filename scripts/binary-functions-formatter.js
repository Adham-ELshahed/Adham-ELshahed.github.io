...
function formatTextProperly(text) {
  ...
  formatted = formatted.replace(/The following functions are used to read and write binary data:/g, 
    'The following functions are used to read and write binary data:\n\n');
  formatted = formatted.replace(/Binary data is represented using the binary type\./g, 
    'Binary data is represented using the binary type.\n\n');
  ...
  // Step 4: Handle binary format specifications (common in binary functions)
  const binaryFormatPatterns = [
    { 
      pattern: 'Compresses a binary value using the given compression type',
      needsBullet: false
    },
    { 
      pattern: 'Decompresses a binary value using the given compression type',
      needsBullet: false
    },
    { 
      pattern: 'Returns a binary value from the given list of numbers',
      needsBullet: false
    },
    { 
      pattern: 'Returns a list of numbers from a binary value',
      needsBullet: false
    }
  ];
  
  // Step 5: Handle encoding/format options that are common in binary functions
  const binaryOptions = [
...
// Filter binary functions
let binaryFunctions = functionsData.filter(func => func.category === 'binary');
let fixedCount = 0;

console.log(`Found ${binaryFunctions.length} binary functions to properly format`);
console.log('Applying proper bullet point formatting with correct line breaks and indentation...\n');

binaryFunctions.forEach((func, index) => {
  console.log(`Formatting ${index + 1}/${binaryFunctions.length}: ${func.name}`);
...
functionsData.forEach((func, index) => {
  if (func.category === 'binary') {
    const formattedFunc = binaryFunctions.find(bf => bf.name === func.name);
    if (formattedFunc) {
...
console.log(`\nCompleted proper formatting for all ${binaryFunctions.length} binary functions`);
